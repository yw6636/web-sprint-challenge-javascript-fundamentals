...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Nested function can have access to the variable internal because of closure. In this case, internal would be lexical scope 
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(data){
    let displayNames = []
    data.forEach (data => displayNames.push(`name: ${data.animal_name}, scientific: ${data.scientific_name}`))
    return displayNames
  }
  ...
  */
  ...
  function lowerCaseNames(data){
    const lowerCase = data.map((element) => {
  ...
  */
  function lowPopulationAnimals(data){
    const lowPopulation = data.filter((element) => {
  ...
  */
  ...
  function USApop(data){
    const totalPop = data.reduce ((total,element) =>{
